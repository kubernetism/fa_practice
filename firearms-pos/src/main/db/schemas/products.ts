...
export const products = sqliteTable(
  'products',
  {
    ...
    make: text('make').$type<'local' | 'imported'>(),
    madeYear: integer('made_year'),
    madeCountry: text('made_country'),
    firearmModelId: integer('firearm_model_id').references(() => firearmModels.id, { onDelete: 'set null' }),
    caliberId: integer('caliber_id').references(() => firearmCalibers.id, { onDelete: 'set null' }),
    shapeId: integer('shape_id').references(() => firearmShapes.id, { onDelete: 'set null' }),
    designId: integer('design_id').references(() => firearmDesigns.id, { onDelete: 'set null' }),
    defaultSupplierId: integer('default_supplier_id').references(() => suppliers.id, { onDelete: 'set null' }),
