...
export function registerChartOfAccountsHandlers() {
  ...
  // Set Opening Balance — books an opening balance JE against an equity offset
  // (default Owner's Capital 3000). Unlike adjust-balance, this ADDS to the
  // current balance rather than retargeting it, and supports a back-dated entry.
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // Resolve offset account: explicit choice, then Owner's Capital 3000,
        // then Retained Earnings 3100, then any equity account.
        const offsetCandidate = offsetAccountId
          ? await db.query.chartOfAccounts.findFirst({
              where: and(
                eq(chartOfAccounts.id, offsetAccountId),
                eq(chartOfAccounts.isActive, true)
              ),
            })
          : (await db.query.chartOfAccounts.findFirst({
              where: and(eq(chartOfAccounts.accountCode, '3000'), eq(chartOfAccounts.isActive, true)),
            })) ||
            (await db.query.chartOfAccounts.findFirst({
              where: and(eq(chartOfAccounts.accountCode, '3100'), eq(chartOfAccounts.isActive, true)),
            })) ||
            (await db.query.chartOfAccounts.findFirst({
              where: and(eq(chartOfAccounts.accountType, 'equity'), eq(chartOfAccounts.isActive, true)),
            }))
        ...
        const result = await withTransaction(async ({ db: txDb }) => {
          ...
          const existing = await txDb.query.journalEntries.findMany({
            where: sql`${journalEntries.entryNumber} LIKE ${'OB-' + year + '-%'}`,
          })
          const entryNumber = `OB-${year}-${String(existing.length + 1).padStart(4, '0')}`

          // For the target account: a positive opening balance increases it,
          // i.e. DR if debit-normal, CR if credit-normal.
          ...
          await txDb.insert(journalEntryLines).values({
            ...
            debitAmount: targetCredit, // mirror
            creditAmount: targetDebit,
          ...
          // Update both balances per their normal-balance convention.
          const targetChange =
            account.normalBalance === 'debit' ? targetDebit - targetCredit : targetCredit - targetDebit
          const offsetChange =
            offsetCandidate.normalBalance === 'debit'
              ? targetCredit - targetDebit
              : targetDebit - targetCredit

          await txDb
            .update(chartOfAccounts)
            .set({ currentBalance: account.currentBalance + targetChange, updatedAt: now })
            .where(eq(chartOfAccounts.id, account.id))
          ...
          await txDb
            ...
            .where(eq(chartOfAccounts.id, offsetCandidate.id))
