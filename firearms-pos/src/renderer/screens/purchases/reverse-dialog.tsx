...
export function ReverseDialog({ purchase, open, onClose, onConfirmed }: Props) {
  ...
  const runPreflight = useCallback(async (purchaseId: number) => {
    const api = (window as unknown as { api: { purchases: { checkReversible: (id: number) => Promise<Preflight> } } }).api
  ...
  const canConfirm =
    preflight?.allowed === true && trimmedReason.length >= 10 && !submitting
      ...
      api: {
        purchases: {
          reverseAndReenter: (
            id: number,
            reason: string,
          ) => Promise<{ success: boolean; error?: string; prefillDraft?: Record<string, unknown> }>
        }
      }
    }).api
