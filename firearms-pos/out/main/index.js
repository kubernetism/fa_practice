  ...
  electron.ipcMain.handle(
    ...
    async (_, params) => {
      try {
        ...
        const globalSetting = await db2.select().from(businessSettings).where(drizzleOrm.sql`${businessSettings.branchId} IS NULL`).limit(1);
...
function generatePDFReceiptHTML(data) {
  ...
  const taxRate = settings2.taxRate || 0;
  ...
  let itemNumber = 0;
  const productItemsHTML = items.map((item) => {
    itemNumber++;
    return `
      <tr>
        ...
        <td class="price-col">${formatCurrency(item.unitPrice, settings2)}</td>
        <td class="qty-col">${item.quantity}</td>
        <td class="total-col">${formatCurrency(item.totalPrice, settings2)}</td>
      </tr>
    `;
  }).join("");
  const serviceItemsHTML = services2.map((service) => {
    itemNumber++;
    return `
      <tr class="service-row">
        ...
        <td class="desc-col">
          ...
          ${service.hours ? `<div class="item-detail">${service.hours} hour${service.hours > 1 ? "s" : ""} @ ${formatCurrency(service.unitPrice, settings2)}/hr</div>` : ""}
          ${service.notes ? `<div class="item-detail">${service.notes}</div>` : ""}
        </td>
        <td class="price-col">${formatCurrency(service.unitPrice, settings2)}</td>
        <td class="qty-col">${service.quantity}</td>
        <td class="total-col">${formatCurrency(service.totalAmount, settings2)}</td>
      </tr>
    `;
  }).join("");
  ...
  const paymentInfoHTML = `
    ...
    ${settings2.receiptCustomField1Label && settings2.receiptCustomField1Value ? `<div class="payment-line">${settings2.receiptCustomField1Label}: ${settings2.receiptCustomField1Value}</div>` : ""}
    ${settings2.receiptCustomField2Label && settings2.receiptCustomField2Value ? `<div class="payment-line">${settings2.receiptCustomField2Label}: ${settings2.receiptCustomField2Value}</div>` : ""}
  `;
  return `
    <!DOCTYPE html>
    <html>
      ...
      <div class="invoice-container">
        ...
        <div class="header">
          <div class="header-left">
            ...
            <div class="business-info">
              <h1>${settings2.businessName || "Business Name Co."}</h1>
        ...
        <div class="footer-section">
          <div class="important-notice">
            ${settings2.receiptTermsAndConditions ? settings2.receiptTermsAndConditions : `<strong>Important:</strong> The invoice amount must be paid no later than 7 business days after issuance.`}
          </div>
          <div class="totals-block">
            <div class="totals-row">
              <span class="totals-label">SUBTOTAL</span>
              <span class="totals-value">: ${formatCurrency(sale.subtotal, settings2)}</span>
            </div>
            ${showTax ? `
            <div class="totals-row">
              <span class="totals-label">TAX (${taxRate}%)</span>
              <span class="totals-value">: ${formatCurrency(sale.taxAmount, settings2)}</span>
            </div>
            ` : ""}
            ${sale.discountAmount > 0 ? `
            <div class="totals-row">
              <span class="totals-label">DISCOUNT</span>
              <span class="totals-value">: -${formatCurrency(sale.discountAmount, settings2)}</span>
            </div>
            ` : ""}
            <div class="totals-row total-final">
              <span class="totals-label">TOTAL</span>
              <span class="totals-value">: ${formatCurrency(sale.totalAmount, settings2)}</span>
            </div>
          </div>
        </div>
        ...
        <div class="balance-warning">
          <strong>Balance Due:</strong> ${formatCurrency(sale.totalAmount - sale.amountPaid, settings2)} — Please settle within the due date.
        </div>
        ` : ""}
        ...
        <div class="payment-section">
          <div class="payment-info">
            ...
            ${paymentInfoHTML}
            ${sale.amountPaid > 0 ? `<div class="payment-line">Amount Paid: ${formatCurrency(sale.amountPaid, settings2)}</div>` : ""}
            ${sale.changeGiven > 0 ? `<div class="payment-line">Change Given: ${formatCurrency(sale.changeGiven, settings2)}</div>` : ""}
          </div>
        ...
        <div class="dev-footer">
          ${settings2.receiptFooter ? `${settings2.receiptFooter}<br><br>` : ""}
